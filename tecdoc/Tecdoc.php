...
if($connection = @mysqli_connect(DBHOST, DBUSER, DBPASS, DB) or die("Нет соединения с БД")){
    echo "connected<br>";
}else{
...
class DBase {
	...
	const USER 	= 'newproject';
	const PASS 	= 'newproject33338';
	const NAME 	= 'ang_tecdoc';
...
//$foo = Tecdoc::getModels(35, 'passenger' );

//$foo = Tecdoc::getModel(5632, 'passenger');
$foo = Tecdoc::getSectionParts( 5631, 101657, 'passenger' );
//$foo = Tecdoc::getArtVehicles( $number, $brand_id );
echo '<pre>';
print_r($foo);
echo '</pre>';